...
- **数量は100株単位**: 追加買い・一部売却とも100株単位で指示する
...
`"$RESULTS_DIR/signals.json"` に含まれる全銘柄を対象とし、以下の優先順で分析する:
...
for t in $(python3 -c "
import yaml, os
with open(os.path.expanduser('~/code/playground/stock-price-analyze/portfolio.yaml')) as f:
    data = yaml.safe_load(f)
print(' '.join(sorted(set(h['ticker'] for h in data.get('holdings', [])))))
"); do
  ~/.claude/skills/stock-advisor/scripts/.venv/bin/python \
    ~/.claude/skills/stock-advisor/scripts/backtest_engine.py --ticker "$t" --strategy all -o "$RESULTS_DIR/backtest/$t.json"
done
# 注目銘柄も同様にバックテスト
for t in $(python3 -c "
import yaml, os
...
if os.path.exists(path):
    with open(path) as f:
        wl = yaml.safe_load(f)
    if wl:
        print(' '.join(w['ticker'] for w in wl))
" 2>/dev/null); do
  ~/.claude/skills/stock-advisor/scripts/.venv/bin/python \
    ~/.claude/skills/stock-advisor/scripts/backtest_engine.py --ticker "$t" --summary -o "$RESULTS_DIR/backtest/$t.json"
done
```
...
`"$RESULTS_DIR/signals.json"` と Step 1 の `portfolio.yaml` をコンテキストとして読み込み、あなた（portfolio-managerロール）が最終取引判断を行う。
