...
```bash
# signal_engineから基準日を取得し、バックテスト終了日を固定（再現性のため）
LATEST_TRADING_DAY=$(python3 -c "
import json
with open('$RESULTS_DIR/signals.json') as f:
    data = json.load(f)
print(data.get('reference_date', ''))
")
...
" 2>/dev/null); do
  ~/.claude/skills/stock-advisor/scripts/.venv/bin/python \
    ~/.claude/skills/stock-advisor/scripts/backtest_engine.py --ticker "$t" \
    --summary --end "$LATEST_TRADING_DAY" -o "$RESULTS_DIR/backtest/$t.json"
done
```
