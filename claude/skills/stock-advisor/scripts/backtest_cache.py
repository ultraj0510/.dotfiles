...
Caches full backtest JSON results keyed by ticker + strategy + start_date + end_date.
...
def compute_config_hash():
    ...
    the signal logic or thresholds have changed.  If signal_rules cannot be
    imported, returns None (all cache reads will be treated as misses).
    """
...
def get_cache_path(ticker, strategy, start_date, end_date):
    """Return cache file path for the given parameters."""
    os.makedirs(CACHE_DIR, exist_ok=True)
    filename = f"{ticker}-{strategy}-{start_date}-{end_date}.json"
    return os.path.join(CACHE_DIR, filename)


def load_cached_result(ticker, strategy, start_date, end_date, max_age=86400):
    ...
    """
    cache_path = get_cache_path(ticker, strategy, start_date, end_date)
...
def save_cached_result(ticker, strategy, start_date, end_date, result):
    """Save backtest result to cache atomically.

    Writes to a temp file then shutil.move to prevent corrupt cache from
    partial writes or concurrent access.  Does NOT mutate the caller's dict.
    """
    cache_path = get_cache_path(ticker, strategy, start_date, end_date)
    ...
    try:
        # Write a copy with config hash injected (don't mutate caller's dict)
        to_write = dict(result)
        to_write["_config_hash"] = compute_config_hash()
        json.dump(to_write, tmp, ensure_ascii=False, indent=2, default=str)
        tmp.flush()
        os.fsync(tmp.fileno())
